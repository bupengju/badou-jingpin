...
class LR(nn.Module):
    ...
    def __init__(self, sentence_lens, vocab_lens, embed_size, hidden_size):
...
def predict(model_path, vocab_path, sentences, embed_size):
    ...
    model = LR(len(sentences[0]), len(vocab), embed_size, 2 * embed_size)
...
def main():
    ...
    model = LR(sentence_lens, len(vocab), embed_size, 2 * embed_size)
    ...
    torch.save(model.state_dict(), "./out/rnn_model.pth")
...
if __name__ == '__main__':
    ...
    predict("./out/rnn_model.pth", "./out/vocab.json", test_sentences, 5)
